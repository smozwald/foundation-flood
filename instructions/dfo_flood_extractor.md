...
2. **DFO per-station URL pattern** — inspect the station list CSV for a URL
   ...
   generalising.
3. **HydroSHEDS coverage** — It is possible that the surrounds are less than the 3x3km bounding box or downstream we don't have anything. We should handle errors such as these (we can basically have less big bounding box or have we can exclude where no data is possible11)
